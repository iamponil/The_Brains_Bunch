...
export default function Basics({ roundedHeaderButton }) {
  ...
  const UpdateSend = async (formData) => {
    const res = await Axios.post(
      `http://localhost:5000/projects/updateProject/${titre}`,
      formData,
      { headers }
    )
      .then(() => {
        ...
        window.location.href = `/Analytics/${titre}`;
      })
      .catch((error) => {
        setErrors(error);
  ...
  const calculateSuggestedGoal = () => {
    const subtotal =
      ...
      (parseFloat(goalAmount) * parseFloat(taxes || 0)) / 100;
    setSubtotaltaxes(subtotal);
    const total =
      subtotal + subtotal * RecycleEngineFee + subtotal * ProcessingFee;
    setSuggestedGoal(total.toFixed(2));
  };
  const selectAmount = () => {
    if (suggestedGoal) setTotal(suggestedGoal);
    handleClose();
    setGoalAmount('');
    setTaxes('');
  };
              ...
              <form onSubmit={(e) => onsubmit(e)} encType="multipart/form-data">
                  ...
                  <Column>
                    ...
                    <InputContainer>
                      <select
                        ...
                        onChange={(e) => hundelchange(e)}
                        tw="sm:pr-48 pl-8 text-primary-500 py-4 sm:py-5 rounded-full border-2 w-full font-medium focus:outline-none transition duration-300  focus:border-primary-500 hover:border-gray-500"
                      >
                    ...
                    <InputContainer>
                      ...
                      <Input
                        ...
                        value={
                          ...
                          !isNaN(new Date(project.launchingDate))
                            ? new Date(project.launchingDate)
                                .toISOString()
                                .slice(0, 10)
                            : ''
                        }
                ...
                <TwoColumn>
                  ...
                  <Column>
                    <br></br>
                    <InputContainer>
                      <Label htmlFor="email-input" tw="text-primary-500">

...
exports.addUser = async (req, res, next) => {
  ...
  bcrypt.genSalt(10, (err, salt) => {
    bcrypt.hash(user.password, salt, async function (err, hash) {
      ...
      user.password = hash;
       // Add the address reference to the user document
...
//     if (req.body.address) {
//       if (req.body.address.streetAdress != null) {
//         user.address.streetAdress = req.body.address.streetAdress;
//       }
...
//     // Return the updated user with the populated address
//     const populatedUser = await updatedUser.populate("address");
//     res.json(updatedUser);
//   } catch (error) {
//     res.status(400).json({ message: error.message });
//   }
...
exports.updateUserAddress = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ address: address._id }).populate("address");
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }
    ...
    if (req.body.address) {
      if (req.body.address.streetAdress != null) {
        address.streetAdress = req.body.address.streetAdress;
      }
      if (req.body.address.city != null) {
...
//     if (categorie == null) {
//       return res.status(400).json({ message: "Insert a valid Category" });
//     }

//     if (user == null) {
//       return res.status(400).json({ message: "Insert a valid user" });
//     }
...
//       user: req.body.user,
//     });
//     const productbeforePopulate = await product.save();
//     try {
//       newProduct = await productbeforePopulate.populate(
//         "categorie complaint user"
//       );
//     } catch (error) {
//       console.log(error);
//     }

//     categorie.product.push(newProduct);
